...
class Board_post extends CB_Controller
{
	...
	public function lists($brd_key = '')
	{
		...
		$fdafa = $list = $this->_get_list($brd_key, '', 'recent');
		
		$config = array(
			'skin' => 'mobile',
			'brd_key' => element('brd_key', $brd_key),
			'limit' => 5,
			'length' => 40,
			'is_gallery' => '',
			'image_width' => '',
		...
		$view['view']['recent'] = $this->board->latest($config, 'data');
	 ...
	 * 게시판 목록페이지입니다.
	 */
	public function _get_list($brd_key, $from_view = '', $mode='')
	{
		...
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				...
				$result['list'][$key]['post_content'] = $result['list'][$key]['post_content'];
				//내용을 100글자 이하로 자른다.
		...
		$config['cur_tag_open'] = '<b class="page-link active">';
